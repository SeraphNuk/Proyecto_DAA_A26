    ...
    for i in range(filas):
        for j in range(columnas-1):
            ...
            if i<=filas-2:
                ...
                grafo.agregar_arista(origen,destinodiaginv)
                grafo.agregar_arista(origendiaginv,destinodiaginv)
                grafo.agregar_arista(origen,destinodiag)

            if j==columnas-2:
                origenvert=f"{i}_{j+1}"
                if i<filas-1:
                    destinovert=f"{i+1}_{j+1}"
                    grafo.agregar_arista(origenvert,destinovert)
    ...
    for i in posiciones_nodos:
        ...
        for j in posiciones_nodos:
            ...
            if distancia>0.0 and distancia<=r and not grafo.existe_arista(nodo_origen,nodo_destino):
                grafo.agregar_arista(nodo_origen,nodo_destino)
...
def Dorogovstev_Mendes(grafo,num_nodos):
    ...
    for i in range(3,num_nodos):
        ...
        if not grafo.existe_arista(i,nodo_a):
            grafo.agregar_arista(i,nodo_a)
        if not grafo.existe_arista(i,nodo_b):
            grafo.agregar_arista(i,nodo_b)
